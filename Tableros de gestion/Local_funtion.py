...
def scalarHxH(dt_serie, hour_serie=None):
    """
    Returns a list containing the specific Hour where an event happened
    Args:
...
def apply_scalars(df_, timestamp_col, month_format='str', shift_format_str=True, prefix=None, suffix=None,
    ...
    #Podemos modificar los turnos en función al rol que se tenga en la determinada empresa
    ...
    if year or month or week or weekday or day or teams_df is not None:
        day_serie = [np.nan if pd.isnull(valH) else \
                     val.strftime('%Y/%m/%d') if valH>='06:00:00' else \
                     (val-dt.timedelta(days=1)).strftime('%Y/%m/%d') \
    ...
    if teams_df is not None:
        tmp_teams_df = pd.DataFrame({
            prefix+'DATE'+suffix: [val.strftime('%Y/%m/%d') for val in teams_df['CAL_DATE']],
    ...
    if year or month or week or weekday:
        ...
        dt_unique_day_serie = pd.to_datetime(unique_day_serie)
